...
import { deleteUserCart, setUserCart } from 'src/contexts/shopping';
...
const Reservation = () => {
  ...
  if (hotelId) {
    ...
    for (let key in queryData) {
      ...
      dataMap[index][keyCopy] = queryData[key];
    }
  ...
  const handleSubmit = async () => {
    data.forEach(async selectedRoom => {
      ...
      const response = await authDispatch(deleteUserCart(selectedRoom.cartId));
